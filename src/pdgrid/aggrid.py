...
class AgGridRequest(GridRequest):
    ...
    FILTER_OPERATORS = {
        ...
        ("text", "contains"): lambda column, filter_value: column.str.contains(
            filter_value
        ),
        ("text", "notContains"): lambda column, filter_value: ~column.str.contains(
            filter_value
        ),
        ("text", "equals"): operator.eq,
        ("text", "notEquals"): operator.ne,
        ("text", "startsWith"): lambda column, filter_value: column.str.startswith(
            filter_value
        ),
        ("text", "endsWith"): lambda column, filter_value: column.str.endswith(
            filter_value
        ),
    ...
    def filter_to_func(self, f):
        if "operator" in f:
            ...
            return column_filter(
                ...
                operator_map["AND"],
                column_filter(
                    self.FILTER_OPERATORS[("number", "greaterThanOrEqual")],
                    f.get("filter"),
                ),
                column_filter(
                    self.FILTER_OPERATORS[("number", "lessThan")], f.get("filterTo")
                ),
            )
            ...
            return column_filter(
                self.FILTER_OPERATORS[(f["filterType"], f.get("type"))],
                f.get("values" if f["filterType"] == "set" else "filter"),
            )
    ...
    def aggregation_params(self):
        return {
            f.get("field"): self.agg_lookup.get(f.get("aggFunc"), f.get("aggFunc"))
            for f in self.request.get("valueCols")
            if f.get("aggFunc") is not None and f.get("field") is not None
        }

    @property
    def sort_columns(self):
        return {
            f.get("colId"): f.get("sort") == "asc"
            for f in self.request.get("sortModel", [])
    ...
    def filter_model(self):
        return {
            field: self.filter_to_func(f)
            for field, f in self.request.get("filterModel", {}).items()
        }

    @property
    def expanded_column_filters(self):
        return {
            field: column_filter(operator.eq, filter_value)
            for field, filter_value in zip(self.groupby_columns, self.selected_groups)
        }
    ...
    def sort_agg_params(self):
        ...
        return {
            f: ("min" if asc else "max")
            for f, asc in self.sort_columns.items()
